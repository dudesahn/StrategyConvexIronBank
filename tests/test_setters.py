...
def test_setters(gov, token, vault, new_address, dudesahn, strategist, whale, strategyProxy, gaugeIB, rando, chain, amount, StrategyCurveIBVoterProxy, live_strategy, vault_balance, strategist_ms):
   ...
   # Update deposit limit to the vault since it's currently maxed out
    ...
    strategy.setCrvRouter(0, {"from": strategist_ms})
    assert strategy.crvRouter != 1

    strategy.setVoter(new_address, {"from": strategist_ms})
    assert strategy.voter() == new_address

    strategy.setTendsPerHarvest(5, {"from": strategist_ms})
